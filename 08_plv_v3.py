"""
08_plv.py — v3 (revision of `08_plv_v2.py`)
─────────────────────────────────────────────────────────────────────────────
Inter-Brain PLV Analysis — Friend vs. Non-Friend (scalar approach)

v3 = v2 + two patches from `08_plv_v2-output-review.md`:

  (A) N_MIN trial-count filter (output-review §1).
      Pairs with `n_aligned < N_MIN` are dropped. PLV's random-phase
      baseline scales as ≈ √(π/4N), so pairs with few aligned trials
      (e.g. Triad 302 with n_aligned=10 → expected PLV ≈ 0.28)
      mechanically inflate the scalar and confound the friend / non-
      friend contrast. N_MIN=30 keeps the random-phase baseline ≤ 0.16.

  (B) EXCLUDE_TRIADS list (output-review §2).
      Triad 330 has all three pairs but two of them hold identical data
      (likely a metadata mismapping — see `08_plv-review.md §3`) and
      the third produces a self-correlation PLV of ~0.4. It survives
      v2's "incomplete pair set" filter and inflates the non-friend
      side of the test. Listed here as a temporary exclusion until the
      metadata-mapping fix lands. **Do not interpret this as the bug
      being resolved — see `08_plv-review.md §3`.**

Inherits from v2:
  - selection-based trial alignment (finding 1)
  - triad-level aggregation, paired Wilcoxon (finding 2)
  - corrected channel_plv reduction (finding 4)
  - filter-then-crop, phase cache, optional trial-shuffle null (N_PERM=0)

Still deferred:
  - Channel-pair averaging dilution (finding 6) — all-pairs scalar.
  - T3P / T3Pn pooling (finding 7) — pooled, single condition set.
  - The underlying Triad 330 metadata bug (finding 3) — masked, not fixed.
─────────────────────────────────────────────────────────────────────────────
"""
...
OUTPUT_DIR = (
    r"C:\Users\clara\OneDrive - Danmarks Tekniske Universitet\Skrivebord\DTU"
    r"\Human Centeret Artificial Intelligence\Thesis\figures\plv_v3"
)
...
# so low-N pairs produce inflated PLV that is not biology.
N_MIN = 30
