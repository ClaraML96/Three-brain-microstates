...
#           The title states whether a significant cluster backs the map.
# ─────────────────────────────────────────────────────────────────────────────
JOINT_SPEC = [
    ("solo_feedback",   ("alpha", "beta")),
    ("trio_feedback",   ("alpha", "beta")),
...
def _evoked_from_diff(r):
    """Wrap a contrast's mean ERD/ERS difference (A − B) as an EvokedArray.
    ...
    info = info_ref.copy()
    with info._unlock():
        info["sfreq"] = 1.0 / np.median(np.diff(times_plot))
...
def plot_joint_diff(contrast_label, band_name, contrast_title):
    ...
    fig = ev.plot_joint(
        ...
        ts_args=dict(scalings=dict(eeg=1.0),
                     ...
                     ylim=dict(eeg=[-45, 45])),
        topomap_args=dict(scalings=dict(eeg=1.0), cmap="RdBu_r", contours=0),
        show=False,
    )
    fname = os.path.join(OUTPUT_DIR,
                         f"wholescalp_joint_{contrast_label}_{band_name}.png")
    fig.savefig(fname, dpi=300, bbox_inches="tight")
    print(f"Saved: {fname}")
    plt.close(fig)


for contrast_label, bands in JOINT_SPEC:
    title = next((t for lab, _, _, t in ec.CONTRASTS if lab == contrast_label),
                 contrast_label)
    for band_name in bands:
        plot_joint_diff(contrast_label, band_name, title)
