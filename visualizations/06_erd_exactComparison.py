...
for epoch_file in EPOCH_FILES:
    ...
    # Derive participant id from filename (prefix before first underscore)
    base = os.path.basename(epoch_file)
    pid = base.split('_')[0]
    if pid not in ALLOWED_PARTICIPANTS:
        print(f"Skipping {base}: participant {pid} not in allowed list")
        continue
