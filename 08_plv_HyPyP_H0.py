...
# be redrawn without re-running. H0 = per-permutation most-extreme cluster mass
# over BOTH bands (two-sided, compared on |H0|). See the primary pipeline.
np.save(os.path.join(OUTPUT_DIR, "plv_overtime_H0.npy"), H0)
print(f"Permutation null H0 → {os.path.join(OUTPUT_DIR, 'plv_overtime_H0.npy')}  "
      f"({len(H0)} values)")

# ═════════════════════════════════════════════════════════════════════════════
# STEP 8 — Figures (t-map + participation + null histogram), same as primary
# ═════════════════════════════════════════════════════════════════════════════
...
def plot_null_histogram():
    """Level-1 inference view: the max-statistic permutation null H0 as a
    histogram, with the observed largest cluster mass and the 95th-percentile
    line marked. ONE histogram — two-sided max-statistic over the alpha+beta
    family — so the marked value is the single largest real cluster overall. For
    a null the observed line falls in the BULK, not the tail. Plotted on |H0|
    because the tail=0 p-value compares absolute values. Mirrors the primary
    pipeline's plot_null_histogram(); complements, does not replace, the N power
    caveat. See ../08_plv_matrix-output-review.md §3."""
    ...
    ax.set_title(f"HyPyP over-time PLV — permutation null (H0) vs. observed · "
                 f"{PLV_CONDITIONS} · N={len(triad_ids)} triads · {len(H0)} perms\n"
                 f"two-sided max-statistic over the alpha + beta family",
                 fontsize=10)
    ax.legend(fontsize=9, frameon=False, loc="upper right")
    plt.tight_layout()
    fn = os.path.join(OUTPUT_DIR, "plv_overtime_null_histogram.png")
    fig.savefig(fn, dpi=300, bbox_inches="tight"); plt.close(fig)
    print(f"Saved: {fn}")
...
print("  • plv_overtime_null_histogram.png   — permutation null vs. observed (Level-1)")
print("  • plv_overtime_H0.npy               — the permutation null distribution")
